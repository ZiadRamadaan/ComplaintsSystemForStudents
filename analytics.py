...
colors = ["#03265b", "#416445", "#38b6ff", "#f39c12", "#8e44ad"]
...
def show_analytics(conn, texts):
    ...
    if complaints_data:
        ...
        kpi2.metric(label="✅ Resolved", value=resolved)
        ...
        with row1_col1:
            fig_cat = px.histogram(df, x="category", color="category",
                                   title="Complaints by Category",
                                   color_discrete_sequence=colors)
            fig_cat.update_layout(title_font_size=18, title_font_color="#2c3e50", xaxis_title=None)
            st.plotly_chart(fig_cat, use_container_width=True)

        with row1_col2:
            fig_pri = px.pie(df, names="priority",
        ...
        with row2_col2:
            ...
            df_monthly = df.groupby("month").size().reset_index(name="count")
            fig_time = px.line(df_monthly, x="month", y="count", markers=True,
                               title="Monthly Complaint Trend",
                               color_discrete_sequence=[colors[0]])
            fig_time.update_layout(title_font_size=18, title_font_color="#2c3e50",
                                   xaxis_title="Month", yaxis_title="Number of Complaints")
            st.plotly_chart(fig_time, use_container_width=True)

    else:
        st.warning("⚠️ No complaint data found in the database.")
